    ...
    <div class="space-y-4">
        ...
        <div class="relative shadow-sm">
          ...
          <div class="z-50 absolute shadow-xl rounded-sm mt-2 border border-gray-200 inset-x-0">
            <ul class="divide-y divide-gray-200 bg-white overflow-hidden">
              ...
              <li wire:click="pickAnime('{{ $result['id'] }}')" class="flex items-center hover:bg-gray-50 hover:cursor-pointer overflow-hidden">
      ...
      <div class="sm:grid grid-cols-3 gap-4">
        <div class="col-span-2">
          ...
          <input wire:model.lazy="anime.title" type="text" name="title" id="title" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-sm mt-2">
        </div>

        <div class="mt-4 sm:mt-0">
          <label for="first_air_date" class="text-sm font-semibold leading-6 text-gray-900">First Air Date</label>
          <input wire:model.lazy="anime.first_air_date" type="date" name="first_air_date" id="first_air_date" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-sm mt-2">
        </div>
      </div>
      ...
      <div class="sm:grid grid-cols-3 gap-4">
        <div class="col-span-2">
          ...
          <input wire:model.lazy="anime.genres" type="text" name="genres" id="genres" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-sm mt-2">
        </div>

        <div class="mt-4 sm:mt-0">
          <label for="tmdb_id" class="text-sm font-semibold leading-6 text-gray-900">TMDB ID</label>
          <input wire:model.lazy="anime.tmdb_id" type="text" name="tmdb_id" id="tmdb_id" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-sm mt-2">
        </div>
      </div>

      <div>
        <label for="overview" class="text-sm font-semibold leading-6 text-gray-900">Overview</label>
        <textarea wire:model.lazy="anime.overview" name="overview" id="overview" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-sm overflow-hidden mt-2" rows="5"></textarea>
      </div>
      ...
      <div>
        ...
        <div class="flex items-center justify-between border border-gray-300 shadow-sm px-3 py-2 mt-2">
          @if (empty($anime->tmdb_poster_path))
          <p class="text-sm">---</p>
          @else
          <a href="#" class="text-sm hover:underline focus:underline focus:outline-none">{{ $anime->tmdb_poster_path }}</a>
          @endif
        </div>
      </div>
      ...
      <div>
        ...
        <div class="flex items-center justify-between border border-gray-300 shadow-sm px-3 py-2 mt-2 space-x-2">
          @if (empty($anime->poster_path))
          <p class="text-sm">---</p>
          @else
          <a href="{{ $anime->poster_url }}" target="_blank" class="text-sm hover:underline focus:underline focus:outline-none truncate">
            {{ $anime->poster_path }}
          </a>
          @endif
      ...
      <div class="sm:grid grid-cols-3 gap-4">
        <div>
          ...
          <input wire:model.lazy="anime.imdb_id" type="text" name="imdb_id" id="imdb_id" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-sm mt-2">
        </div>

        <div class="mt-4 sm:mt-0">
          <label for="imdb_rating" class="text-sm font-semibold leading-6 text-gray-900">IMDB Rating</label>
          <input wire:model.lazy="anime.imdb_rating" type="text" name="imdb_rating" id="imdb_rating" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-sm mt-2">
        </div>

        <div class="mt-4 sm:mt-0">
          <label for="is_approved" class="text-sm font-semibold leading-6 text-gray-900">Approved</label>
          <select wire:model.lazy="anime.is_approved" name="is_approved" id="is_approved" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-sm mt-2">
            <option value="1" {{ (boolean) $anime->is_approved == true ? 'selected' : null }}>Yes</option>
            <option value="0" {{ (boolean) $anime->is_approved == false ? 'selected' : null }}>No</option>
          </select>
        </div>
      </div>
      ...
      <div class="sm:grid grid-cols-3 gap-4">
        <div>
          ...
          <p class="text-sm mt-2">{{ $anime->updated_at_for_human }}</p>
        </div>
