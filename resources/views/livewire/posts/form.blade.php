    ...
    <div class="space-y-4">
      <div x-data="{}">
        ...
        <div class="mt-2 border border-dashed border-gray-200 rounded-sm h-[64px] flex items-center justify-center text-gray-400">
          <p>Add item by <span x-on:click="$refs.search.focus()" class="hover:cursor-pointer hover:underline focus:underline">searching</span></p>
        </div>
        @endif
      </div>
      ...
      <div class="shadow-sm">
        ...
        <input wire:model.lazy="title" type="text" name="title" id="title" maxlength="100" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-sm mt-2" placeholder="Untitled">
      </div>

      <div wire:ignore class="">
        <label for="description" class="text-sm font-semibold leading-6 text-gray-900">Description</label>
        <div x-title="editor" x-data="editor($wire.entangle('description').defer)" class="prose prose-slate lg:prose-lg mt-2">
